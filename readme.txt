=== Radius ===
Contributors: Bizink,Jayden Major
Tags: Bizink, Radius
Requires at least: 5.6
Tested up to: 6.8.3
Stable tag: 1.6.8
Requires PHP: 7.4

The Radius theme

== Description ==
The Radius theme

== Changelog ==

= 1.6.8 =
* Fixed Landing Page 2

= 1.6.7 =
* Fixed Landing Page

= 1.6.6 =
* Support WP Bump
* Spelling
* CSS Fix

= 1.6.5 =
* Fix for Testimonial Grid layout
* Removed redundant page password checks

= 1.6.4 =
* Bug Fix - Toggle for Enabling Testimonials

= 1.6.3 =
* Info Box BG Fix

= 1.6.2 =
* Screen Reader Content Fix

= 1.6.1 =
* List Item Css Fix

= 1.6 =
* Bug Fixes
* Footer Address

= 1.5.9 =
* Blog Posts per page follow setting

= 1.5.8 =
* Info Box 4 Columns
* Update Updater

= 1.5.7 =
* Logo Grid

= 1.5.6 =
* Updater Fix
* Bizpress Style

= 1.5.5 =
* Password Page Update

= 1.5.4 =
* Bug Fixes 2

= 1.5.3 =
* Bug Fixes

= 1.5.2 =
* Password Protected Page Fix

= 1.5.1 =
* Scss Fix

= 1.5 =
* Password Protected Page
* Scss Fix

= 1.4.9 =
* Footer Script Updates

= 1.4.8 =
* Fix for header & footer scripts

= 1.4.7 =
* Version Bump

= 1.4.6 =
* Login Logo

= 1.4.5 =
* Large Team Page
* Social Icons Fix

= 1.4.4 =
* Landing Page Header & Footer
* Mobile Fix for info box

= 1.4.3 =
* Remove Weekly Digest Featured Image

= 1.4.2 =
* Fix Team member link

= 1.4.1 =
* Website Logos Removal
* Skillset title removal

= 1.4 =
* Mobile fixes

= 1.3.9 =
* Words Outline - Subtitle Fix

= 1.3.8 =
* Row Fix

= 1.3.7 =
* Resources Page - Spacing Fix
* Weekly Digest Page Fixes
* Title Background Fix

= 1.3.6 =
* Pagages Updates
* Resources Update
* Testimonial Update

= 1.3.5 =
* Weekly Digest Induvial Page
* Banner Breadcrumb color

= 1.3.4 =
* Weekly Digests Fix

= 1.3.3 =
* Revert Scss

= 1.3.2 =
* SCSS Fix
* Resources Single Page Content Fix
* Weekly Digests Page
* Breadcrumb Titles

= 1.3.1 =
* Testimonial flexable layout section
* Team template flexable layout section

= 1.3 =
* Fix Form selection issue
* Move Newsltter and icons to flexable layout
* Make Client Area menu use the menu title like wave
* Fix Resources Page

= 1.2 =
* Enable Features - Eshentials
* Enable updater
* Footer By Bizink

= 1.1 =
* Update Twitter to X
* Info Boxes Fix
* Two Columns Fix
* Added Counter
* Update Header Client Button

= 1.0 =
* First Realease